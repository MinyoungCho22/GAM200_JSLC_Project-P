...
// ---------------------------------------------------------------------------
static constexpr float ASSUMED_IMG_HEIGHT =
    1080.0f;  // expected car image height in pixels
// Flatbed deck surface: art row py=804 → world offset above Train::MIN_Y (see
// MakeHitbox for [84,804,2472,45]).
static constexpr float kTrainFlatbedDeckTopLocalY = ASSUMED_IMG_HEIGHT - 804.f;
// rail.png: DrawRailTrack는 타일 전체(MIN_Y ~ MIN_Y+m_railTileH)에 맞추지만,
// 궤도 픽셀은 대부분 타일 하단에 있다. 물리 슬랩 윗면 = MIN_Y + m_railTileH *
// 이 값 (현재 아트 기준 0.08).
...
static void ScaleTrainCombatRobot(Robot& r) {
  ...
  r.SetSize({s.x * Train::kCombatDroneVisualScale,
             s.y * Train::kCombatDroneVisualScale});
}
...
static GLuint CompileGLShader(GLenum type, const char* src) {
  ...
  if (!ok) {
    char log[1024];
    GL::GetShaderInfoLog(shader, static_cast<GLsizei>(sizeof(log)), nullptr,
                         log);
    Logger::Instance().Log(Logger::Severity::Error,
                           "Train valve water shader compile failed:\n%s", log);
...
static GLuint LinkGLProgram(GLuint vs, GLuint fs) {
  ...
  if (!ok) {
    char log[1024];
    GL::GetProgramInfoLog(program, static_cast<GLsizei>(sizeof(log)), nullptr,
                          log);
    Logger::Instance().Log(Logger::Severity::Error,
                           "Train valve water program link failed:\n%s", log);
...
namespace {
// GPU-instanced valve water shader (GLSL 330 core — patched for WebGL2 in
// __EMSCRIPTEN__ builds)
...
static Train::TrainHitbox MakeHitbox(
    float carOffset, float px, float py, float pw, float ph,
    bool collision = true,
    Train::TrainHitboxKind kind = Train::TrainHitboxKind::Solid) {
  float cx = carOffset + px + pw * 0.5f;
  float cy = ASSUMED_IMG_HEIGHT - py -
             ph * 0.5f;  // flip Y: pixel-from-top → world-from-bottom
  return {{cx, cy}, {pw, ph}, collision, kind};
}

// Third_Third 자동차 영역: 단일 바깥 박스 (표시용). collision=false면 플레이어
// 통과.
static void AppendCarSilhouette(std::vector<Train::TrainHitbox>& out,
                                float carOffset, float px, float py, float pw,
                                float ph, bool collision) {
  out.push_back(MakeHitbox(carOffset, px, py, pw, ph, collision,
                           Train::TrainHitboxKind::Solid));
}

// Loose world-space band over the train cars for “riding” horizontal carry
// (jump / crouch).
...
static bool HitboxInTrainCarryBand(float trainWorldLeft, float totalTrainWidth,
                                   const Math::Vec2& hbCenter,
                                   const Math::Vec2& hbHalf) {
  constexpr float margin = 56.0f;
  const float l = trainWorldLeft - margin;
  const float r = trainWorldLeft + totalTrainWidth + margin;
  if (hbCenter.x + hbHalf.x < l || hbCenter.x - hbHalf.x > r) return false;
  if (hbCenter.y + hbHalf.y < Train::MIN_Y + 24.0f) return false;
...
// Car4(Third_Third) 이미지 픽셀 — x: 칸 왼쪽 기준, yTop: 이미지 상단에서 아래로
// (+Y 아래)
static const struct {
  float x, yTop;
    ...
    {330.f, 122.f},  {315.f, 292.f},  {360.f, 532.f},  {3380.f, 118.f},
    {3420.f, 288.f}, {3360.f, 528.f},
};
}  // namespace
...
int Train::FindCarTransportInjectTarget(Math::Vec2 playerHbCenter) const {
  ...
  for (int i = 0; i < kCarTransportCount; ++i) {
    if (m_carTransportSlots[static_cast<size_t>(i)].engineOn) continue;
    const Math::Vec2 w = CarTransportWorldCenter(i);
    const float d = (playerHbCenter - w).LengthSq();
    if (d > kCarInteractRangeSq || d >= bestD) continue;
    bestD = d;
    best = i;
  }
  return best;
...
void Train::ResetCarTransportSlotsToInitialState() {
  ...
  for (int i = 0; i < kCarTransportCount; ++i) {
    Train::TrainHitbox hb =
        MakeHitbox(c4, defs[i].px, defs[i].py, defs[i].pw, defs[i].ph, false,
    ...
    m_carTransportSlots[static_cast<size_t>(i)].skipPulseLineOverlay =
        (i == 0 || i == 5);
  }
...
void Train::NotifyCarTransportInjectionInterrupted() {
  for (auto& s : m_carTransportSlots) s.injectPulseAccum = 0.f;
  m_carInjectFocusSlot = -1;
}

bool Train::TryGetCarTransportSkillAnchor(Math::Vec2 playerHbCenter,
                                          Math::Vec2& outWorldAnchor) const {
  int best = -1;
  float bestD = 1.e18f;
  for (int i = 0; i < kCarTransportCount; ++i) {
    const Math::Vec2 w = CarTransportWorldCenter(i);
    const float d = (playerHbCenter - w).LengthSq();
    if (d > kCarSkillAnchorRangeSq || d >= bestD) continue;
    bestD = d;
    best = i;
  }
...
int Train::TryGetCarTransportStartInjectSlot(Math::Vec2 playerHbCenter,
                                             Math::Vec2 playerHbSize,
  ...
  for (int i = 0; i < kCarTransportCount; ++i) {
    ...
    const Math::Vec2 wc = {trainLeft + slot.localCenter.x,
                           MIN_Y + slot.localCenter.y};
    const float d = (playerHbCenter - wc).LengthSq();
    if (d > kCarInteractRangeSq) continue;
...
bool Train::TryGetCarTransportClickIgniteTarget(Math::Vec2 playerHbCenter,
                                                Math::Vec2 playerHbSize,
                                                Math::Vec2 mouseWorldPos,
                                                int& outSlotIndex) const {
  (void)playerHbSize;
  const Math::Vec2 kCursorHitbox{32.f, 32.f};

  for (int i = 0; i < kCarTransportCount; ++i) {
    const auto& slot = m_carTransportSlots[static_cast<size_t>(i)];
    if (slot.engineOn) continue;
    ...
    const bool mouseOnCar =
        Collision::CheckPointInAABB(mouseWorldPos, wc, full) ||
        Collision::CheckAABB(mouseWorldPos, kCursorHitbox, wc, full);
    if (!mouseOnCar) continue;
...
bool Train::TryIgniteCarTransportSlot(int slotIndex) {
  if (slotIndex < 0 || slotIndex >= kCarTransportCount) return false;

  auto& slot = m_carTransportSlots[static_cast<size_t>(slotIndex)];
  if (slot.engineOn) return false;

  slot.engineOn = true;
  slot.injectPulseAccum = 0.f;
  slot.engineGlowTimer = 1.35f;
  if (m_carInjectFocusSlot == slotIndex) m_carInjectFocusSlot = -1;

  FireStraightLineChainDetonations();

  Logger::Instance().Log(Logger::Severity::Info,
                         "Train: Car transport engine ON (click slot %d)",
                         slotIndex);
  return true;
}

void Train::AppendCarTransportStraightChainArcs(
    std::vector<std::pair<Math::Vec2, Math::Vec2>>& outArcs) const {
  constexpr int pairCount = static_cast<int>(sizeof(kStraightCarPairs) /
                                             sizeof(kStraightCarPairs[0]));
...
void Train::AppendCarTransportPulseBranchArcs(
    std::vector<std::pair<Math::Vec2, Math::Vec2>>& outArcs,
    Math::Vec2 pulseWorldCenter, float pulseRadius) const {
  if (!m_carTransportDroneManager) return;
  ...
  for (int s = 0; s < kCarTransportCount; ++s) {
    if (!m_carTransportSlots[static_cast<size_t>(s)].engineOn) continue;
    litSlot[litCount] = s;
  ...
  for (size_t i = 0; i < drones.size(); ++i) {
    ...
    if ((dp - pulseWorldCenter).LengthSq() > pulseR2) continue;
    ...
    if (bestD > kCarDroneMaxSq) continue;
  ...
  std::sort(near.begin(), near.end(), [](const NearD& A, const NearD& B) {
    if (A.carSlot != B.carSlot) return A.carSlot < B.carSlot;
...
void Train::ApplyPulseToTrainRobots(Math::Vec2 pulseWorldCenter, float radius) {
  ...
  for (auto& robot : m_robots) {
    ...
    const Math::Vec2 dir =
        (dist > 0.1f) ? (d * (1.f / dist)) : Math::Vec2{1.f, 0.f};
    const Math::Vec2 impulse =
        dir * (impulseMag * kRobotImpulseScale) + Math::Vec2{0.f, kLift};
...
float Train::GetTrainCarLocalLeftEdge(int carIndex1To5) const {
  switch (carIndex1To5) {
    ...
    case 2:
      return m_car1Width;
    case 3:
      return m_car1Width + m_car2Width;
    case 4:
      return m_car1Width + m_car2Width + m_car3Width;
    case 5:
      return m_car1Width + m_car2Width + m_car3Width + m_car4Width;
    case 6:
      return m_totalTrainWidth;
    default:
      return m_totalTrainWidth;
  }
}
...
int Train::GetPlayerTrainCarIndex(Math::Vec2 worldHbCenter) const {
  ...
  if (lx < 0.f || lx >= m_totalTrainWidth) return 0;
  if (lx < m_car1Width) return 1;
  if (lx < m_car1Width + m_car2Width) return 2;
  if (lx < m_car1Width + m_car2Width + m_car3Width) return 3;
  if (lx < m_car1Width + m_car2Width + m_car3Width + m_car4Width) return 4;
  return 5;
}
...
bool Train::IsTrainCarCombatCleared(int car1To5) const {
  if (car1To5 < 1 || car1To5 > 5) return true;

  if (m_droneManager) {
    for (const auto& d : m_droneManager->GetDrones())
      if (!d.IsDead() && d.GetTrainCarSegment() == car1To5) return false;
  }
  ...
  for (const auto& r : m_robots)
    if (!r.IsDead() && r.GetTrainCarSegment() == car1To5) return false;
  return true;
}
...
float Train::GetTrainCombatAdvanceCapWorldX() const {
  ...
  // Cars 1–3: single zone — move freely until all three are cleared, then cap
  // at car 4.
  const bool zone123Cleared = IsTrainCarCombatCleared(1) &&
                              IsTrainCarCombatCleared(2) &&
                              IsTrainCarCombatCleared(3);
  if (!zone123Cleared)
    return std::min(MIN_X + m_trainOffset + GetTrainCarLocalLeftEdge(4) - 38.f,
                    tailGuard);
  if (!IsTrainCarCombatCleared(4))
    return std::min(MIN_X + m_trainOffset + GetTrainCarLocalLeftEdge(5) - 38.f,
                    tailGuard);
...
std::string Train::GetCar5ValveHintBannerText() const {
  if (m_car5ValveHintTimer <= 0.0f) return {};
  return "Last car reached. Spin the valve to the right — flood the deck and "
         "wash the robots away!";
}

void Train::RequestTrainCameraShake(float maxPixelOffset) {
  m_pendingTrainCameraShakePx =
      std::max(m_pendingTrainCameraShakePx, maxPixelOffset);
}
...
void Train::DrawRobotTrainAlerts(Shader& colorShader,
                                 DebugRenderer& debugRenderer) const {
  for (const auto& robot : m_robots) {
    if (!robot.IsDead()) robot.DrawTrainDetectAlert(colorShader, debugRenderer);
  }
}

void Train::TryDeckPatrolRobotJumpAndLandingShake(
    Robot& r, size_t robotIndex, float dt,
    const std::vector<ObstacleInfo>& tallCarObs, int playerTrainCar) {
  if (r.IsDead() || !r.IsTrainDeckPatrol() ||
      robotIndex >= m_trainDeckRobotWasAirborne.size())
    return;
  if (robotIndex >= m_trainDeckRobotJumpPrepT.size()) return;
  ...
  if (!airborne && m_trainDeckRobotWasAirborne[robotIndex]) {
    if (playerTrainCar == r.GetTrainCarSegment() &&
        robotIndex < m_trainDeckRobotUsedLandingShake.size() &&
  ...
  if (r.GetVelocity().y > 25.f) return;

  // No deck vault/jump (match Underground robots; avoids Car2 purple container
  // snagging / flicker).
  (void)tallCarObs;
}
...
void Train::ClampCarSegment4RobotsBeforeValve() {
  ...
  for (auto& r : m_robots) {
    if (r.IsDead() || r.GetTrainCarSegment() != 4) continue;
    Math::Vec2 p = r.GetPosition();
    if (p.x <= capX) continue;
    p.x = capX;
    r.SetPosition(p);
...
void Train::ClampCar5RobotsEastOfValve() {
  // 밸브 중심보다 왼쪽(작은 X)으로 못 가게 — 마지막 칸 로봇은 밸브 오른쪽
  // 덱에만
  ...
  for (auto& r : m_robots) {
    if (r.IsDead() || r.GetTrainCarSegment() != 5) continue;
    Math::Vec2 p = r.GetPosition();
    if (p.x >= minRobotCx) continue;
    p.x = minRobotCx;
    r.SetPosition(p);
    Math::Vec2 v = r.GetVelocity();
    if (v.x < 0.f) v.x = 0.f;
...
void Train::UpdateTrainDeckPatrolRobots(float dt, Player& player,
                                        Math::Vec2 playerHbCenter,
                                        Math::Vec2 playerHitboxSize) {
  ...
  for (size_t ri = 0; ri < m_robots.size(); ++ri) {
    Robot& r = m_robots[ri];
    if (r.IsDead() || !r.IsTrainDeckPatrol()) continue;

    const int seg = r.GetTrainCarSegment();
    if (seg != 1 && seg != 2 && seg != 4 && seg != 5) continue;
    ...
    for (const auto& hb : m_trainHitboxes) {
      ...
      if (hb.kind == TrainHitboxKind::JumpThroughPipe) continue;
      if (hb.size.y < minHbH) continue;
      ...
      if (wxR < carWorldL || wxL > carWorldR) continue;
    ...
    if (seg == 4) {
      ...
      r.SetPatrolWorldClamp(std::max(carWorldL + 90.f, minPatrolX),
                            carWorldR - 90.f);
    } else
    ...
    if (seg == 2 && m_car2PurpleHbValid && !r.IsDead()) {
      ...
      const Math::Vec2 pc = {tl + m_car2PurpleHb.localCenter.x,
                             MIN_Y + m_car2PurpleHb.localCenter.y};
    ...
    TryDeckPatrolRobotJumpAndLandingShake(r, ri, static_cast<float>(dt),
                                          carTall, pcar);
...
bool Train::IsValveMouseHoverable(Math::Vec2 playerHbCenter,
                                  Math::Vec2 playerHbSize,
                                  Math::Vec2 mouseWorldPos) const {
  const Math::Vec2 valveWorld = {MIN_X + m_trainOffset + m_valveLocalCenter.x,
                                 MIN_Y + m_valveLocalCenter.y};
  // Interaction range widened so player can rotate from noticeably farther
  // away.
  if (!Collision::CheckAABB(playerHbCenter, playerHbSize, valveWorld,
                            {560.0f, 320.0f}))
    return false;

  return Collision::CheckPointInAABB(mouseWorldPos, valveWorld,
                                     m_valveVisualSize) ||
         Collision::CheckAABB(mouseWorldPos, {32.0f, 32.0f}, valveWorld,
                              m_valveVisualSize);
}

bool Train::IsPointOverWorldCollisionAABB(Math::Vec2 worldPos,
                                          Math::Vec2 cursorHitboxSize) const {
  ...
  for (const auto& hb : m_trainHitboxes) {
    ...
    if (test(wc, hb.size)) return true;
  }
...
void Train::UpdateValveWaterParticles(float dt) {
  ...
  for (auto& p : m_valveWaterParticles) {
    ...
    if (p.life > 0.0f && p.vel.y < -120.0f && p.maxLife > 0.45f &&
        prevY >= floorY && p.pos.y < floorY) {
      const int splashCount = 3 + static_cast<int>(m_valvePressureT * 6.0f);
      for (int i = 0; i < splashCount; ++i) {
        const float fi = static_cast<float>(i);
        const float ph = m_valveWaterAnimTime * 6.5f + fi * 1.37f +
                         static_cast<float>(m_valveParticleCounter % 29u);
  ...
  m_valveWaterParticles.erase(
      std::remove_if(m_valveWaterParticles.begin(), m_valveWaterParticles.end(),
                     [floorY](const ValveWaterParticle& p) {
                       return p.life <= 0.0f || p.pos.y < floorY;
                     }),
      m_valveWaterParticles.end());
  if (!m_valveSplashScratch.empty())
    m_valveWaterParticles.insert(m_valveWaterParticles.end(),
                                 m_valveSplashScratch.begin(),
                                 m_valveSplashScratch.end());

  // ApplyValveWaterDamageToEnemies는 Train::Update 끝에서 호출 — 스크립트가
  // SetPosition으로 드론/로봇을 배치한 뒤에 넉백·데미지가 적용되도록 한다.
  ...
  while (m_valveParticleSpawnCarry >= 1.0f) {
    ...
    const float ph = static_cast<float>(m_valveParticleCounter) * 0.91f +
                     m_valveWaterAnimTime * 2.4f;
    ...
    if (dirSign > 0.0f) outletX += 42.0f;  // right nozzle feels more "open"
    p.pos = {valveWorld.x + outletX + outletJitterX,
             valveWorld.y - 40.0f + outletJitterY};

    const float pressureSpeed = 80.0f + m_valvePressureT * 360.0f;
    float sideKick =
        dirSign * (180.0f + m_valvePressureT * 420.0f + std::sin(ph) * 64.0f);
    if (dirSign > 0.0f)
      sideKick *= 2.15f;  // right stream: much stronger horizontal push

    float vertical = -pressureSpeed + std::cos(ph * 1.7f) * 32.0f;
    if (dirSign < 0.0f) vertical *= 0.50f;  // slightly flatter than before
    if (dirSign > 0.0f)
      vertical *= 0.26f;  // right stream: slightly lower launch angle
    p.vel = {sideKick, vertical};

    const float s =
        18.0f + m_valvePressureT * 24.0f + std::sin(ph * 0.8f) * 3.4f;
    const float sMul = (dirSign > 0.0f) ? 1.22f : 1.12f;
    p.size = {s * 1.12f * sMul, s * 1.60f * sMul};
    p.maxLife = 0.55f + m_valvePressureT * 0.55f +
                std::abs(std::sin(ph * 0.6f)) * 0.18f;
    p.life = p.maxLife;
  ...
  if (m_valveWaterParticles.size() > kMaxValveWaterParticles) {
    const std::size_t drop =
        m_valveWaterParticles.size() - kMaxValveWaterParticles;
    m_valveWaterParticles.erase(
...
void Train::ApplyMooreConnectedPulseShare(Player& player, float dt) {
    ...
    while (head < tail) {
      ...
      for (int v = 0; v < kCarTransportCount; ++v) {
        if (!m_carTransportSlots[static_cast<size_t>(v)].engineOn || visited[v])
          continue;
        if (!IsMooreAdjacentCarSlots(u, v)) continue;
        visited[v] = true;
        q[tail++] = v;
      }
    }
  ...
  if (bonus > 0.f) player.GetPulseCore().getPulse().add(bonus);
}

void Train::FireStraightLineChainDetonations() {
  if (!m_droneManager && !m_carTransportDroneManager) return;
  ...
  for (int e = 0; e < pairCount; ++e) {
    ...
    if (m_droneManager)
      m_droneManager->ApplyDetonation(mid, kChainDetonateRadius,
                                      kChainDetonateStun);
    if (m_carTransportDroneManager)
      m_carTransportDroneManager->ApplyDetonation(mid, kChainDetonateRadius,
                                                  kChainDetonateStun);
  }
}

void Train::UpdateCarTransport(float dt, Player& player,
                               Math::Vec2 playerHbCenter, bool injectHeld,
                               bool ignorePulseCost, int forcedInjectSlot) {
  const float fdt = dt;
  ...
  if (forcedInjectSlot >= 0 && forcedInjectSlot < kCarTransportCount) {
    ...
    if (!fs.engineOn) {
      ...
      const float d = (playerHbCenter - w).LengthSq();
      if (d <= kCarInteractRangeSq) focus = forcedInjectSlot;
    }
  }
  ...
  if (!injectHeld || focus < 0) {
    if (m_carInjectFocusSlot >= 0) {
      m_carTransportSlots[static_cast<size_t>(m_carInjectFocusSlot)]
          .injectPulseAccum = 0.f;
      m_carInjectFocusSlot = -1;
    }
    return;
  }
  ...
  const float pulsePerSec =
      kCarTransportPulseInjectTotal / kCarTransportPulseInjectDuration;
...
void Train::UpdateCarTransportDrones(float dt, const Player& player,
                                     Math::Vec2 playerHitboxSize,
  ...
  for (size_t i = 0; i < drones.size(); ++i) {
    Drone& d = drones[i];
    if (static_cast<int>(i) < kCarTransportHoverDroneCount &&
        d.IsCarTransportPersistHover()) {
    ...
    if (static_cast<int>(i) < kCarTransportHoverDroneCount &&
        d.IsCarTransportPersistHover() && !d.IsDead() && !d.IsHit() &&
        !d.IsCarTransportAggroChase())
...
void Train::Initialize() {
  ...
  // File name in request had spacing typo ("Valve. png"), so try common
  // variants.
  ...
  if (m_firstTrain->GetWidth() > 0)
    m_car1Width = static_cast<float>(m_firstTrain->GetWidth());
  if (m_secondTrain->GetWidth() > 0)
    m_car2Width = static_cast<float>(m_secondTrain->GetWidth());
  if (m_thirdTrain->GetWidth() > 0)
    m_car3Width = static_cast<float>(m_thirdTrain->GetWidth());
  if (m_thirdThirdTrain->GetWidth() > 0)
    m_car4Width = static_cast<float>(m_thirdThirdTrain->GetWidth());
  if (m_fourthTrain->GetWidth() > 0)
    m_car5Width = static_cast<float>(m_fourthTrain->GetWidth());
  m_totalTrainWidth =
      m_car1Width + m_car2Width + m_car3Width + m_car4Width + m_car5Width;

  // Car5 valve anchor: centered on existing valve/pipe hitbox (c5,
  // 894,351,317,162). Keep local-space so it follows train offset
  // automatically.
  {
  ...
  // How many rail tiles needed to cover train width (plus a few extra for
  // safety)
  m_railTileCount =
      static_cast<int>(std::ceil(m_totalTrainWidth / m_railTileW)) + 4;
  ...
  // User requested "TrainStart.mpe" and "TrainSound.mp3". Keep .mpe first, then
  // fallback to .mp3.
  ...
  if (runLoaded) m_trainRunLoopSound.SetVolume(0.0f);

  ApplyConfig(MapObjectConfig::Instance().GetData().train);

  // 드론: 1~4호차 각 2기 + 5호차 6기; Car4(카트랜스포트)는 덱 로봇 없음 ·
  // 1·2·5호차 덱 패트롤 + 5호차 광폭 2
  {
    for (auto& r : m_robots) r.Shutdown();
    m_robots.clear();
    if (m_droneManager) m_droneManager->ClearAllDrones();
    ...
    for (int carIdx = 1; carIdx <= 5; ++carIdx) {
      ...
      if (carIdx == 5) {
        const float ySiren = m_car3SirenHbValid
                                 ? (MIN_Y + m_car3SirenHb.localCenter.y + 55.f)
                                 : highCarDroneHoverY;
        constexpr int kCar5DroneCount = 6;
        // 물탱크 뒤(차량 앞쪽 / Car4 경계 쪽)와 앞(열차 끝 쪽)으로 나눠 배치,
        // 속도·Y도 제각각
        for (int j = 0; j < kCar5DroneCount; ++j) {
          ...
          const float xj = MIN_X + l + w * t +
                           static_cast<float>((j * 23 + 5) % 13 - 6) * 11.f;
          const float yExtra = behindTank
                                   ? (-18.f - static_cast<float>(sub) * 14.f)
                                   : (12.f + static_cast<float>(sub) * 16.f);
          Drone& dj = m_droneManager->SpawnDrone({xj, ySiren + yExtra},
                                                 "Asset/Drone.png", false);
          ScaleTrainCombatDrone(dj);
          const float spd = behindTank
                                ? (62.f + static_cast<float>(sub) * 19.f)
      ...
      Drone& da =
          m_droneManager->SpawnDrone({xA, yA}, "Asset/Drone.png", false);
      ScaleTrainCombatDrone(da);
      Drone& db =
          m_droneManager->SpawnDrone({xB, yB}, "Asset/Drone.png", false);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      const float cx = MIN_X + c5 + m_car5Width - 92.f -
                       static_cast<float>(i) * 126.f + phaseX;
    ...
    for (int b = 0; b < 2; ++b) {
      ...
      Robot& rr = m_robots.back();
      const float bx =
          MIN_X + c5 + m_car5Width * (0.38f + static_cast<float>(b) * 0.16f);
    ...
    if (m_carTransportDroneManager) {
      ...
      for (int i = 0; i < kCarTransportHoverDroneCount; ++i) {
        ...
        Drone& dd = m_carTransportDroneManager->SpawnDrone(
            {wx, wy}, "Asset/Drone.png", false);
      ...
      m_carTransportDroneWaterCd.assign(
          m_carTransportDroneManager->GetDrones().size(), 0.f);
    }
  }
  ...
  Logger::Instance().Log(
      ...
      "Train Map initialized – car widths: %.0f / %.0f / %.0f / %.0f / %.0f "
      "(total %.0f), rail tile: %.0f — drones %zu robots %zu",
      m_car1Width, m_car2Width, m_car3Width, m_car4Width, m_car5Width,
      m_totalTrainWidth, m_railTileW, m_droneManager->GetDrones().size(),
      m_robots.size());
}
...
//    pixelY     : 해당 오브젝트의 이미지 내 상단 Y 픽셀 위치 (↑ 작을수록 위, ↓
//    클수록 아래) width      : 히트박스 가로 크기 (픽셀 단위) height     :
//    히트박스 세로 크기 (픽셀 단위)
//
...
void Train::BuildTrainHitboxes() {
  ...
  //    이미지 내 오브젝트 배치 (좌 → 우)
  //    [발판] - [컨테이너A(빨강)] - [컨테이너B(은색, A 위)] - [컨테이너C(황갈)]
  //    - [컨테이너D(은색, 우측)]
  // ════════════════════════════════════════════════════════════════════════
  ...
  //   위치: X=204  Y=525  크기: 501 x 285
  m_trainHitboxes.push_back(MakeHitbox(c2, 204, 525, 501, 285));

  // [Car2] 컨테이너 F  ─  소형 어두운 컨테이너 (발판 위, 낮은 층)
  //   위치: X=1434  Y=519  크기: 207 x 286
  m_trainHitboxes.push_back(MakeHitbox(c2, 1434, 519, 207, 286));

  // [Car2] 컨테이너 G  ─  대형 보라색 컨테이너 (높은 층 – 가장 높은 위치 중
  // 하나)
  //   위치: X=1653  Y=234  크기: 603 x 285
  ...
  //   위치: X=2268  Y=519  크기: 183 x 285
  m_trainHitboxes.push_back(MakeHitbox(c2, 2268, 519, 183, 285));

  // ════════════════════════════════════════════════════════════════════════
  // ▣  Car 3  –  ThirdTrain.png
  //    [발판] - [컨테이너I(은색/흰)] - [컨테이너J(어두운 대형)] -
  //    [컨테이너K(황갈, 위)] - [컨테이너L(소형, 우측)]
  // ════════════════════════════════════════════════════════════════════════
  ...
  m_trainHitboxes.push_back(
      MakeHitbox(c4, 429.0f, 74.0f, 3086.0f, kPipeH, true,
                 Train::TrainHitboxKind::JumpThroughPipe));

  // 2층 파이프: X=432 ~ X=3507 (왼쪽 차 시작 ~ 오른쪽 차 끝), 폭 = 3075
  m_trainHitboxes.push_back(
      MakeHitbox(c4, 432.0f, 434.0f, 3075.0f, kPipeH, true,
                 Train::TrainHitboxKind::JumpThroughPipe));

  constexpr bool kCar4VisualOnly = false;
  AppendCarSilhouette(m_trainHitboxes, c4, 429, 129, 886, 304,
                      kCar4VisualOnly);  // top-left
  AppendCarSilhouette(m_trainHitboxes, c4, 1536, 129, 886, 304,
                      kCar4VisualOnly);  // top-middle
  AppendCarSilhouette(m_trainHitboxes, c4, 2629, 129, 886, 304,
                      kCar4VisualOnly);  // top-right

  AppendCarSilhouette(m_trainHitboxes, c4, 432, 489, 876, 306,
                      kCar4VisualOnly);  // bottom-left
  AppendCarSilhouette(m_trainHitboxes, c4, 1539, 489, 876, 306,
                      kCar4VisualOnly);  // bottom-middle
  AppendCarSilhouette(m_trainHitboxes, c4, 2631, 489, 876, 306,
                      kCar4VisualOnly);  // bottom-right
  ...
  m_trainHitboxes.push_back(
      MakeHitbox(c5, 342, 498, 63, 307));  // tank left end
  m_trainHitboxes.push_back(
      MakeHitbox(c5, 405, 498, 948, 307));  // tank main body
  m_trainHitboxes.push_back(
      MakeHitbox(c5, 1353, 498, 62, 307));  // tank right end
  m_trainHitboxes.push_back(
      MakeHitbox(c5, 894, 351, 317, 162));  // valve / pipe on top
  ...
  // rail.png — Draw와 동일한 타일 박스 안에서 실제 궤도
  // 높이(kRailWalkSurfaceFractionOfTileH)에 발판 정렬. localCenter: 맵
  // 원점(MIN_X, MIN_Y) 기준 오프셋 (열차 히트박스와 동일).
  m_staticWorldHitboxes.clear();
  {
    ...
    const float walkY = std::clamp(tileH * kRailWalkSurfaceFractionOfTileH,
                                   10.0f, std::max(12.0f, tileH - 4.0f));
...
void Train::InitValveWaterGpu() {
  ...
  GL::DeleteShader(vs);
  GL::DeleteShader(fs);
  if (!m_valveWaterProg) return;

  m_valveWaterLocProjection =
      GL::GetUniformLocation(m_valveWaterProg, "projection");
  ...
  GL::BufferData(GL_ARRAY_BUFFER,
                 static_cast<GLsizeiptr>(m_valveWaterInstPoolBytes), nullptr,
                 GL_DYNAMIC_DRAW);

  GL::VertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, instStride,
                          (void*)offsetof(ValveWaterGpuInstance, center));
  GL::EnableVertexAttribArray(1);
  GL::VertexAttribDivisor(1, 1);

  GL::VertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, instStride,
                          (void*)offsetof(ValveWaterGpuInstance, halfSize));
  GL::EnableVertexAttribArray(2);
  GL::VertexAttribDivisor(2, 1);

  GL::VertexAttribPointer(3, 1, GL_FLOAT, GL_FALSE, instStride,
                          (void*)offsetof(ValveWaterGpuInstance, alpha));
  GL::EnableVertexAttribArray(3);
  GL::VertexAttribDivisor(3, 1);

  GL::VertexAttribPointer(4, 1, GL_FLOAT, GL_FALSE, instStride,
                          (void*)offsetof(ValveWaterGpuInstance, layer));
...
void Train::ShutdownValveWaterGpu() {
  if (m_valveWaterVAO) {
    GL::DeleteVertexArrays(1, &m_valveWaterVAO);
    m_valveWaterVAO = 0;
  }
  if (m_valveWaterQuadVBO) {
    GL::DeleteBuffers(1, &m_valveWaterQuadVBO);
    m_valveWaterQuadVBO = 0;
  }
  if (m_valveWaterInstVBO) {
    GL::DeleteBuffers(1, &m_valveWaterInstVBO);
    m_valveWaterInstVBO = 0;
  }
  if (m_valveWaterProg) {
    GL::DeleteProgram(m_valveWaterProg);
    m_valveWaterProg = 0;
  }
...
void Train::UploadAndDrawValveWaterGpu(const Math::Matrix& projection,
                                       Math::Vec2 cameraPos,
                                       float viewHalfW) const {
  if (!m_valveWaterGpuReady || !m_valveWaterProg || !m_valveWaterVAO) return;
  ...
  for (const auto& p : m_valveWaterParticles) {
    if (p.pos.x < visL || p.pos.x > visR) continue;

    const float lifeT =
        (p.maxLife > 0.0f) ? std::clamp(p.life / p.maxLife, 0.0f, 1.0f) : 0.0f;
    ...
    const Math::Vec2 mainSize = {p.size.x * widthMul * rightOnlyBoost,
                                 p.size.y * stretch * 1.20f};
  ...
  if (m_valveWaterGpuScratch.empty()) return;

  const GLsizeiptr bytesNeeded = static_cast<GLsizeiptr>(
      m_valveWaterGpuScratch.size() * sizeof(ValveWaterGpuInstance));

  GL::BindBuffer(GL_ARRAY_BUFFER, m_valveWaterInstVBO);
  if (bytesNeeded > static_cast<GLsizeiptr>(m_valveWaterInstPoolBytes)) {
    const GLsizeiptr newBytes =
        (bytesNeeded * 3) / 2 +
        static_cast<GLsizeiptr>(sizeof(ValveWaterGpuInstance)) * 256;
  ...
  GL::BufferSubData(GL_ARRAY_BUFFER, 0,
                    static_cast<GLsizeiptr>(m_valveWaterGpuScratch.size() *
                                            sizeof(ValveWaterGpuInstance)),
                    m_valveWaterGpuScratch.data());
  GL::BindBuffer(GL_ARRAY_BUFFER, 0);

  GL::UseProgram(m_valveWaterProg);
  GL::UniformMatrix4fv(m_valveWaterLocProjection, 1, GL_FALSE,
                       projection.Ptr());

  GL::BindVertexArray(m_valveWaterVAO);
  GL::DrawArraysInstanced(GL_TRIANGLES, 0, 6,
                          static_cast<GLsizei>(m_valveWaterGpuScratch.size()));
  GL::BindVertexArray(0);
  GL::UseProgram(0);
}
...
void Train::ApplyValveWaterDamageToEnemies(float dt) {
  if (dt <= 0.0f) return;
  if (m_valvePressureT <= 0.01f && m_valveWaterParticles.empty()) return;
  ...
  auto hitsWater = [&](const Math::Vec2& enemyCenter,
                       const Math::Vec2& enemySize,
                       float extraEnemyPadding) -> bool {
    if (m_valveWaterParticles.empty()) return false;
    for (const auto& p : m_valveWaterParticles) {
      if (p.life <= 0.0f) continue;

      const float lifeT = (p.maxLife > 0.0f)
      ...
      const Math::Vec2 mainSize = {p.size.x * widthMul * jetBoost * hitBoost,
                                   p.size.y * stretch * 1.20f};
      if (!Collision::CheckAABB(p.pos, mainSize, enemyCenter, paddedEnemySize))
        continue;

      if (m_valvePressureT < 0.05f && lifeT < 0.05f) continue;
  ...
  // 밸브가 열린 동안: 탱크 우측 저공 구역을 물줄기로 간주 (파티클이 얇을 때도
  // 피격 판정 보강)
  auto inSprayBand = [&](const Math::Vec2& enemyCenter,
                         const Math::Vec2& enemySize) -> bool {
    if (m_valvePressureT < 0.05f) return false;
    ...
    if (enemyCenter.x < xL || enemyCenter.x > xR) return false;
    // 물탱크 발판·저공 드론 높이대
    if (feet > MIN_Y + 420.f || head < MIN_Y + 40.f) return false;
    return true;
  };
  ...
  if (m_droneManager) {
    ...
    for (size_t i = 0; i < drones.size(); ++i) {
      ...
      if (i < m_droneWaterCd.size() && m_droneWaterCd[i] > 0.0f) continue;

      const bool wet = hitsWater(d.GetPosition(), d.GetSize(), 36.0f) ||
                       inSprayBand(d.GetPosition(), d.GetSize());
      if (!wet) continue;

      // Train 맵은 Drone::Update를 돌리지 않아 ApplyKnockback 슬라이드가 보이지
      // 않음 → 위치로 즉시 넉백
      ...
      d.TakeDamage(dmg);
      if (i < m_droneWaterCd.size()) m_droneWaterCd[i] = 1.15f;
    }
  }
  ...
  if (m_carTransportDroneManager) {
    ...
    for (size_t i = 0; i < ctd.size(); ++i) {
      ...
      if (i < m_carTransportDroneWaterCd.size() &&
          m_carTransportDroneWaterCd[i] > 0.0f)
...
bool Train::IsPlayerOnCar5Deck(Math::Vec2 hbCenter, Math::Vec2 hbSize) const {
  if (!m_car5DeckHbValid) return false;

  const float tl = MIN_X + m_trainOffset;
  Math::Vec2 deckWorld = {tl + m_car5DeckHb.localCenter.x,
                          MIN_Y + m_car5DeckHb.localCenter.y};
  Math::Vec2 deckSize = m_car5DeckHb.size;
  // 착지·판정 안정화: 발판 위로 약간 확장 (히트박스가 발판과 스킵되는 경우
  // 방지)
  deckSize.y += 72.f;
  deckWorld.y += 36.f;
...
void Train::TryActivateCar5Encounter(Math::Vec2 hbCenter, Math::Vec2 hbSize) {
  if (m_car5EncounterActive) return;

  bool shouldActivate = IsPlayerOnCar5Deck(hbCenter, hbSize);
  if (!shouldActivate) {
    // Deck top 외에도 탱크/밸브 위에 올라탄 경우 활성화되도록 Car5 구역 체크를
    // 허용.
    const float tl = MIN_X + m_trainOffset;
    const float c5 = m_car1Width + m_car2Width + m_car3Width + m_car4Width;
    ...
    shouldActivate =
        (hbCenter.x >= car5L && hbCenter.x <= car5R &&
         hbCenter.y >= MIN_Y + 120.f && hbCenter.y <= MIN_Y + HEIGHT + 260.f);
  }
  if (!shouldActivate) return;

  m_car5EncounterActive = true;
  if (!m_trainCheatCarUnlock) m_car5ValveHintTimer = 5.0f;
  m_encounterScriptTime = 0.f;
  ...
  if (!m_robots.empty()) {
    const float tl = MIN_X + m_trainOffset;
    const float c4Right =
        tl + m_car1Width + m_car2Width + m_car3Width + m_car4Width;
    const float rail = Train::MIN_Y + 95.f;
    int railSlot = 0;
    for (size_t i = 0; i < m_robots.size(); ++i) {
      Robot& rr = m_robots[i];
      if (rr.IsTrainDeckPatrol()) continue;
  ...
  Logger::Instance().Log(Logger::Severity::Event,
                         "Train: FourthTrain deck encounter activated");
}

void Train::AssistRobotRailJumpTowardTrain(Robot& robot, const Player& player,
                                           float dt) {
  (void)dt;
  ...
  if (feet > Train::MIN_Y + 230.f) return;
  if (pp.y < rp.y + 90.f) return;
  if (robot.GetVelocity().y > 320.f) return;

  const RobotState st = robot.GetState();
  if (st != RobotState::Chase && st != RobotState::Patrol &&
      st != RobotState::Windup && st != RobotState::Recover)
    return;

  if (std::abs(robot.GetVelocity().x) > 72.f) return;
...
void Train::UpdateTrainRobotsAI(float dt, Player& player) {
  if (!m_car5EncounterActive || m_robots.empty()) return;
  ...
  for (auto& r : m_robots) {
    if (r.IsDead()) continue;
    if (r.IsTrainDeckPatrol()) continue;
    ...
    if (!r.IsTrainDeckPatrol()) AssistRobotRailJumpTowardTrain(r, player, dt);
  }
...
void Train::UpdateCar3Siren(float dt, Player& player, Math::Vec2 playerHbCenter,
                            Math::Vec2 playerHitboxSize,
                            Math::Vec2 mouseWorldPos, bool attackHeld,
                            bool injectGodMode) {
  if (!m_car3SirenHbValid || !m_sirenDroneManager) return;
  ...
  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 sirenW = {tl + m_car3SirenHb.localCenter.x,
                             MIN_Y + m_car3SirenHb.localCenter.y};

  m_car3SirenWaveAnim += dt * 5.5f;

  const bool inInjectRange = Collision::CheckAABB(
      playerHbCenter, playerHitboxSize, sirenW, {380.f, 300.f});
  const bool cursorOnSiren =
      Collision::CheckPointInAABB(mouseWorldPos, sirenW, m_car3SirenHb.size);

  if (m_car3SirenActive) {
    const bool playerOnCar3 = (GetPlayerTrainCarIndex(playerHbCenter) == 3);
    if (playerOnCar3) m_car3SirenSpawnTimer += static_cast<float>(dt);
    if (playerOnCar3 && m_car3SirenSpawnTimer >= 2.4f &&
        m_sirenDroneManager->GetDrones().size() < 14) {
      m_car3SirenSpawnTimer = 0.f;
      const Math::Vec2 spawnPos = {sirenW.x + 25.f, sirenW.y + 55.f};
      Drone& d =
          m_sirenDroneManager->SpawnDrone(spawnPos, "Asset/Drone.png", true);
    ...
    if (attackHeld && inInjectRange && cursorOnSiren) {
      ...
      if (!injectGodMode) {
        pulseThisFrame =
            std::min(pulseThisFrame, player.GetPulseCore().getPulse().Value());
        if (pulseThisFrame > 0.f)
          player.GetPulseCore().getPulse().spend(pulseThisFrame);
      }
      if (pulseThisFrame > 0.f || injectGodMode) {
        const float deltaT = injectGodMode
                                 ? (static_cast<float>(dt) / kInjectDurationSec)
  ...
  const float speedRatio =
      (TRAIN_SPEED > 0.f)
          ? std::clamp(m_trainCurrentSpeed / TRAIN_SPEED, 0.f, 1.f)
          : 0.f;
  constexpr float kSirenTracerTrainAssistK = 0.22f;
  const float trainAssist = 1.f + kSirenTracerTrainAssistK * speedRatio;
  m_sirenDroneManager->Update(dt, player, playerHitboxSize, hideForSiren, true,
                              chaseMul, trainAssist);
}

bool Train::IsCar3SirenMouseHoverForPulseInject(Math::Vec2 playerHbCenter,
                                                Math::Vec2 playerHbSize,
                                                Math::Vec2 mouseWorld) const {
  if (!m_car3SirenHbValid || !m_car3SirenActive) return false;
  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 sirenW = {tl + m_car3SirenHb.localCenter.x,
                             MIN_Y + m_car3SirenHb.localCenter.y};
  if (!Collision::CheckAABB(playerHbCenter, playerHbSize, sirenW,
                            {380.f, 300.f}))
    return false;
  return Collision::CheckPointInAABB(mouseWorld, sirenW, m_car3SirenHb.size);
}

bool Train::IsPlayerInCar2PurplePulseBox(Math::Vec2 playerHbCenter,
                                         Math::Vec2 playerHitboxSize) const {
  if (!m_car2PurpleHbValid || m_car2HidePhase != Car2HidePhase::Inside)
    return false;
  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 contC = {tl + m_car2PurpleHb.localCenter.x,
                            MIN_Y + m_car2PurpleHb.localCenter.y};
  return Collision::CheckAABB(playerHbCenter, playerHitboxSize, contC,
                              m_car2PurpleHb.size);
}

bool Train::IsSirenDroneDamageBlocked(Math::Vec2 playerHbCenter,
                                      Math::Vec2 playerHitboxSize,
...
bool Train::IsCar2PurpleHitbox(const TrainHitbox& hb) const {
  if (!m_car2PurpleHbValid) return false;
...
void Train::UpdateCar2PurpleContainer(float dt, Player& player,
                                      Math::Vec2 mouseWorldPos,
                                      bool attackTriggered,
                                      bool pulseAbsorbHeld,
                                      bool injectGodMode) {
  if (!m_car2PurpleHbValid) return;

  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 contC = {tl + m_car2PurpleHb.localCenter.x,
                            MIN_Y + m_car2PurpleHb.localCenter.y};

  // 플레이어 히트박스 중심을 컨테이너 중심에 고정 — 중력/걷기로 박스 아래로
  // 떨어지지 않음
  ...
  const bool nearPurple = Collision::CheckAABB(
      player.GetHitboxCenter(), player.GetHitboxSize(), contC,
      {m_car2PurpleHb.size.x + 520.f, m_car2PurpleHb.size.y + 520.f});

  const Math::Vec2 promptCenter = {
      contC.x - m_car2PurpleHb.size.x * 0.5f - 115.f, contC.y + 10.f};
  const Math::Vec2 promptSize = {150.f, 78.f};
  const bool hoverPrompt =
      Collision::CheckPointInAABB(mouseWorldPos, promptCenter, promptSize) &&
      nearPurple;
  ...
  switch (m_car2HidePhase) {
    case Car2HidePhase::None:
      ...
      if (nearPurple && attackTriggered && hoverPrompt &&
          m_car2InsideCharge < 99.5f) {
    ...
    case Car2HidePhase::Inside: {
      ...
      player.GetPulseCore().getPulse().add(pulsePerSec *
                                           static_cast<float>(dt));
      m_car2InsideCharge =
          std::min(100.f, m_car2InsideCharge +
                              chargeMeterPerSec * static_cast<float>(dt));
      ...
      if (nearPurple && attackTriggered && hoverPrompt &&
        ...
        if (m_car2EnterSavedValid) {
          ...
          player.SetPosition(
              {m_car2EnterPlayerWorld.x + dOff, m_car2EnterPlayerWorld.y});
...
void Train::DrawCar3SirenWaves(Shader& colorShader, Math::Vec2 cameraPos,
                               float viewHalfW) const {
  if (!m_car3SirenHbValid || !m_car3SirenActive || m_car3SirenInjectT >= 0.995f)
    return;

  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 o = {tl + m_car3SirenHb.localCenter.x,
                        MIN_Y + m_car3SirenHb.localCenter.y + 70.f};

  const float visLeft = cameraPos.x - viewHalfW - 400.f;
  const float visRight = cameraPos.x + viewHalfW + 400.f;
  if (o.x < visLeft || o.x > visRight) return;

  for (int i = 0; i < 4; ++i) {
    const float ph = m_car3SirenWaveAnim * 2.6f + static_cast<float>(i) * 1.15f;
    const float wave = std::fmod(ph, 6.2831853f);
    const float rad =
        90.f + static_cast<float>(i) * 68.f + std::sin(wave) * 22.f;
    const float a = 0.14f + static_cast<float>(i) * 0.05f;
    DrawFilledQuad(colorShader, o, {rad * 2.2f, rad * 0.65f}, 0.55f, 0.15f,
                   0.95f, a * (1.f - m_car3SirenInjectT));
  }
}

void Train::DrawCar2EnterLeavePrompt(Shader& textureShader,
                                     Math::Vec2 cameraPos,
                                     float viewHalfW) const {
  if (!m_car2PurpleHbValid) return;
  ...
  const float visLeft = cameraPos.x - viewHalfW - 400.f;
  const float visRight = cameraPos.x + viewHalfW + 400.f;
  if (promptCenter.x < visLeft || promptCenter.x > visRight) return;

  Background* tex = nullptr;
  if (m_car2HidePhase == Car2HidePhase::Inside && m_car2InsideCharge >= 99.5f &&
    ...
    tex = m_car2LeavePromptTex.get();
            ...
            m_car2HidePhase == Car2HidePhase::Entering) &&
           m_car2InsideCharge < 99.5f && m_car2EnterPromptTex &&
           m_car2EnterPromptTex->GetWidth() > 0)
  ...
  Math::Matrix model = Math::Matrix::CreateTranslation(promptCenter) *
                       Math::Matrix::CreateScale({pw * 0.85f, ph * 0.85f});
  tex->Draw(textureShader, model);
}

bool Train::IsCar2EnterLeavePromptHovered(Math::Vec2 playerHbCenter,
                                          Math::Vec2 playerHbSize,
  ...
  const bool nearPurple = Collision::CheckAABB(
      playerHbCenter, playerHbSize, contC,
      {m_car2PurpleHb.size.x + 520.f, m_car2PurpleHb.size.y + 520.f});
  ...
  const float visLeft = cameraPos.x - viewHalfW - 400.f;
  const float visRight = cameraPos.x + viewHalfW + 400.f;
  if (promptCenter.x < visLeft || promptCenter.x > visRight) return false;

  return nearPurple &&
         Collision::CheckPointInAABB(mouseWorld, promptCenter, promptSize);
}

void Train::DrawCar3SirenProgressGauge(Shader& colorShader,
                                       Math::Vec2 cameraPos,
                                       float viewHalfW) const {
  if (!m_car3SirenHbValid || !m_car3SirenActive || !m_skyVAO) return;

  const float tl = MIN_X + m_trainOffset;
  const Math::Vec2 posC = {tl + m_car3SirenHb.localCenter.x,
                           MIN_Y + m_car3SirenHb.localCenter.y};
  ...
  const float barH =
      std::min(110.0f, std::max(44.0f, m_car3SirenHb.size.y * 0.9f));
  ...
  const float visLeft = cameraPos.x - viewHalfW - 400.f;
  const float visRight = cameraPos.x + viewHalfW + 400.f;
  if (barCenterX < visLeft || barCenterX > visRight) return;

  colorShader.use();

  const Math::Vec2 bgSize = {barW + 6.0f, barH + 6.0f};
  DrawFilledQuad(colorShader, {barCenterX, innerCenterY}, bgSize, 0.2f, 0.2f,
                 0.2f, 1.0f);

  DrawFilledQuad(colorShader, {barCenterX, innerCenterY}, {barW, barH}, 0.14f,
                 0.14f, 0.18f, 1.0f);
  ...
  if (fillH > 0.5f) {
    ...
    DrawFilledQuad(colorShader, {barCenterX, fillCenterY}, {barW, fillH}, 0.8f,
                   0.2f, 1.0f, 1.0f);
  }
}

void Train::DrawCarTransportInjectProgressGauge(Shader& colorShader,
                                                Math::Vec2 cameraPos,
                                                float viewHalfW) const {
  if (!m_skyVAO) return;
  ...
  const auto& slot =
      m_carTransportSlots[static_cast<size_t>(m_carInjectFocusSlot)];
  if (slot.engineOn) return;

  const float trainLeft = MIN_X + m_trainOffset;
  const Math::Vec2 posC = {trainLeft + slot.localCenter.x,
                           MIN_Y + slot.localCenter.y};
  ...
  const float t = std::clamp(
      slot.injectPulseAccum / kCarTransportPulseInjectTotal, 0.f, 1.f);
  const float fillH = barH * t;
  if (fillH > 0.5f) {
    const float innerBottomY = innerTopY - barH;
    const float fillCenterY = innerBottomY + fillH * 0.5f;
    DrawFilledQuad(colorShader, {barCenterX, fillCenterY}, {barW, fillH}, 0.25f,
                   0.85f, 1.0f, 1.0f);
  }
}
...
void Train::UpdateTrainEncounterScript(float dt, Player& player) {
  ...
  if (m_droneManager) {
    ...
    for (size_t i = 0; i < drones.size(); ++i) {
      auto& d = drones[i];
      if (d.IsDead()) continue;
      if (d.IsHit() || d.IsStunned()) continue;
      if (d.GetTrainCarSegment() != 5) continue;

      Math::Vec2 pos = d.GetPosition();
      const float bobY = std::sin(m_encounterScriptTime * 2.45f +
                                  static_cast<float>(i) * 0.73f) *
                         5.f;
      ...
      if (fleeWater) {
        ...
        if (pos.x + dx > capR) dx = std::max(0.f, capR - pos.x);
      } else {
        const float spacing =
            formationSpacingBase + static_cast<float>(car5LiveIdx % 6) * 31.f;
        const float slot = static_cast<float>(car5LiveIdx) - 2.5f;
        const float desiredX = std::clamp(formationCenter + slot * spacing,
                                          c5L + 110.f, c5R - 110.f);
        const float ap =
            kCar5DroneApproach + static_cast<float>(car5LiveIdx % 5) * 19.f;
...
// ---------------------------------------------------------------------------
bool Train::IsPlayerHiding(Math::Vec2 playerPos, Math::Vec2 playerHitboxSize,
                           bool isPlayerCrouching) const {
  if (!isPlayerCrouching) return false;

  const float trainWorldLeft = MIN_X + m_trainOffset;
  for (const auto& spot : m_hidingSpots) {
    Math::Vec2 worldPos = {trainWorldLeft + spot.localCenter.x,
                           MIN_Y + spot.localCenter.y};
...
// ---------------------------------------------------------------------------
// ResolveJumpThroughAABB – thin “pipe” tiers on Third_ThirdTrain: pass upward,
// land from above, crouch = fall through.
// ---------------------------------------------------------------------------
static bool ResolveJumpThroughAABB(Player& player, Math::Vec2& currentHbCenter,
                                   const Math::Vec2& playerHalfSize,
                                   const Math::Vec2& obsCenter,
                                   const Math::Vec2& obsSize, bool crouchHeld) {
  ...
  if (feet > platTop + kTierAboveSlab) return false;

  // 빠르게 상승 중이면 무조건 통과.
  // (2층 파이프에서 점프 시작 프레임에 같은 파이프가 다시 스냅해 점프를 막는
  // 문제 방지)
  constexpr float kRiseVy = 52.0f;
  if (vy > kRiseVy) return false;
...
static bool SnapToTopSupport(Player& player, Math::Vec2& currentHbCenter,
  ...
  if (kind == Train::TrainHitboxKind::Solid &&
      IsThinHorizontalTrainSlab(obsSize)) {
    constexpr float kEdgeInset = 15.f;
    if (currentHbCenter.x < obsMin.x + kEdgeInset ||
        currentHbCenter.x > obsMax.x - kEdgeInset)
      return false;
  }
  ...
  if (vy > 60.0f) return false;

  const float aboveTol =
      (kind == Train::TrainHitboxKind::JumpThroughPipe) ? 10.0f : 8.0f;
  // 아래에서 점프해 덱에 올라오기 쉽게(떨어진 뒤 재탑승).
  const float belowTol =
      (kind == Train::TrainHitboxKind::JumpThroughPipe) ? 72.0f : 88.0f;

  if (feet > platTop + aboveTol || feet < platTop - belowTol) return false;
...
//   Returns true if the player was pushed UP onto the surface (standing on
//   top).
// ---------------------------------------------------------------------------
...
static bool ResolveAABB(Player& player, Math::Vec2& currentHbCenter,
                        ...
                        const Math::Vec2& obsCenter,
                        const Math::Vec2& obsSize) {
  if (!Collision::CheckAABB(currentHbCenter, playerHalfSize * 2.0f, obsCenter,
                            obsSize))
    return false;
  ...
  // 얇은 가로 발판: overlapY가 작을 때 overlapX < overlapY 로 가로 밀림이 나와
  // 웅크리기 낙하 시 옆으로 튐 → 세로 착지만
  if (IsThinHorizontalSlab(obsSize)) {
    ...
    const bool centerOnSlab = currentHbCenter.x >= obsMin.x + kDeckEdgeInset &&
                              currentHbCenter.x <= obsMax.x - kDeckEdgeInset;
    // 공중·점프 상승 중에도 덱 아래에서 넓은 범위로 착지 허용(재탑승). 덱 좌우
    // 끝 가장자리는 자석 현상 방지.
    if (horizOverlap && centerOnSlab && vy <= 260.0f &&
        feet <= platTop + 64.0f && feet >= platTop - 168.0f) {
      Math::Vec2 newHbCenter = currentHbCenter;
      newHbCenter.y = platTop + playerHalfSize.y;
      Math::Vec2 shift = newHbCenter - currentHbCenter;
      player.SetPosition(player.GetPosition() + shift);
...
void Train::Update(double dt, Player& player, Math::Vec2 playerHitboxSize,
                   bool pulseAbsorbHeld, bool carTransportInjectHeld,
                   bool ignoreCarInjectPulseCost, bool attackHeld,
                   bool attackTriggered, Math::Vec2 mouseWorldPos,
  ...
  const bool playerInSubway = playerPos.y >= MIN_Y &&
                              playerPos.y <= MIN_Y + HEIGHT + 400.0f &&
  ...
  // Valve clockwise drag interaction (Car5 top valve) -> tank spill VFX.
  {
    const Math::Vec2 valveWorld = {MIN_X + m_trainOffset + m_valveLocalCenter.x,
                                   MIN_Y + m_valveLocalCenter.y};
    const bool canHover = IsValveMouseHoverable(
        player.GetHitboxCenter(), playerHitboxSize, mouseWorldPos);

    if (!attackHeld) m_valveDragging = false;
  ...
  UpdateCar2PurpleContainer(fdt, player, mouseWorldPos, attackTriggered,
                            pulseAbsorbHeld, ignoreCarInjectPulseCost);
  UpdateCar3Siren(fdt, player, player.GetHitboxCenter(), playerHitboxSize,
                  mouseWorldPos, attackHeld, ignoreCarInjectPulseCost);

  // --- Drone / Robot: FourthTrain 스크립트 (Car5 발판 진입 전까지 정지) ---
  const bool isPlayerHiding = IsPlayerHiding(
      player.GetHitboxCenter(), playerHitboxSize, player.IsCrouching());
  const bool inCar2PulseBox =
      IsPlayerInCar2PurplePulseBox(player.GetHitboxCenter(), playerHitboxSize);
  ...
  UpdateTrainDeckPatrolRobots(fdt, player, player.GetHitboxCenter(),
                              playerHitboxSize);

  if (m_droneManager)
    m_droneManager->Update(fdt, player, playerHitboxSize, trainEnemyUndetect,
                           true, 1.f);

  if (m_car5EncounterActive)
    UpdateTrainEncounterScript(fdt, player);
  else if (m_droneManager) {
    // 인카운터 전: 1·2·3호차 전투 드론은 Drone::Update만 사용(칸 간 추적·Q
    // 넉백). 4호차 저공 호버, 5호차 성형.
    auto& drones = m_droneManager->GetDrones();
    ...
    const auto trackXInRange = [&](float left, float right, float wanderFreq,
                                   size_t idx) -> float {
      if (!trainEnemyUndetect) return std::clamp(pTarget.x, left, right);
      const float mid = 0.5f * (left + right);
      const float half = 0.5f * (right - left) - 40.f;
      if (half <= 8.f) return mid;
      return mid + std::sin(m_encounterScriptTime * wanderFreq +
                            static_cast<float>(idx) * 0.91f) *
                       half * 0.88f;
    };
    ...
    for (size_t i = 0; i < drones.size(); ++i) {
      ...
      if (seg == 4) {
        ...
        p.y = baseY + std::sin(m_encounterScriptTime * 2.1f +
                               static_cast<float>(i) * 0.78f) *
                          20.f;
        d.SetPosition(p);
        d.SetVelocity({0.f, 0.f});
        d.SetBaseSpeed(95.f);
      ...
      if (seg == 5) {
        const float ySiren = m_car3SirenHbValid
                                 ? (MIN_Y + m_car3SirenHb.localCenter.y + 55.f)
                                 : car12DroneHoverBase;
        const float hoverY = ySiren + std::sin(m_encounterScriptTime * 2.2f +
                                               static_cast<float>(i) * 0.61f) *
                                          5.f;
        Math::Vec2 p = d.GetPosition();
        const float desiredX =
            trackXInRange(c5L + 100.f, c5R - 100.f, 2.95f, i);
        const float ap = 118.f + static_cast<float>(i % 5) * 11.f;
        p.x = std::clamp(p.x + std::clamp(desiredX - p.x, -ap * fdt, ap * fdt),
                         c5L + 100.f, c5R - 100.f);
        p.y = hoverY;
        d.SetPosition(p);
        d.SetVelocity({0.f, 0.f});
  ...
  // ─── 파이프 드롭스루 (S 누르는 순간) ───
  // collision pass 전에 직접 검사: 파이프 top과 발 차이가 작으면 즉시 -100px
  // nudge. 이렇게 하면 SnapToTopSupport / ResolveJumpThroughAABB의 race 조건과
  // 무관하게 확실히 드롭된다.
  ...
  if (crouchPressed && m_pipeDropCooldown <= 0.0f) {
    ...
    for (const auto& hb : m_trainHitboxes) {
      ...
      const bool xOverlap =
          (hc.x + playerHalfSize.x > xMin) && (hc.x - playerHalfSize.x < xMax);
      if (!xOverlap) continue;
      if (std::abs(feet - top) > 24.0f)
        continue;  // 파이프 top 근처에 발이 있어야 함

      constexpr float kPipeDropNudge = 100.0f;
      player.SetPosition(player.GetPosition() +
                         Math::Vec2{0.0f, -kPipeDropNudge});
  ...
  for (const auto& hb : m_trainHitboxes) {
    ...
    Math::Vec2 hbWorld = {trainWorldLeft + hb.localCenter.x,
                          MIN_Y + hb.localCenter.y};

    bool landed = false;
    if (hb.kind == Train::TrainHitboxKind::JumpThroughPipe) {
      if (m_pipeDropCooldown > 0.0f)
        continue;  // 드롭 직후엔 파이프 다시 잡지 않음
      landed = ResolveJumpThroughAABB(player, currentHbCenter, playerHalfSize,
                                      hbWorld, hb.size, crouchHeld);
    } else
      landed = ResolveAABB(player, currentHbCenter, playerHalfSize, hbWorld,
                           hb.size);
  ...
  // if collision solver didn't mark landed this frame, keep support when feet
  // are already on a top surface.
  if (!playerOnTrainSurface) {
    for (const auto& hb : m_trainHitboxes) {
      ...
      if (hb.kind == Train::TrainHitboxKind::JumpThroughPipe &&
          m_pipeDropCooldown > 0.0f)
        continue;
      if (SnapToTopSupport(player, currentHbCenter, playerHalfSize, hbWorld,
                           hb.size, hb.kind, crouchHeld)) {
        playerOnTrainSurface = true;
        if (hb.kind == Train::TrainHitboxKind::JumpThroughPipe)
          playerOnJumpThroughSurface = true;
        break;
  ...
  for (const auto& hb : m_staticWorldHitboxes) {
    if (!hb.collision) continue;
    const Math::Vec2 hbWorld = {MIN_X + hb.localCenter.x,
                                MIN_Y + hb.localCenter.y};
    const bool landed =
        ResolveAABB(player, currentHbCenter, playerHalfSize, hbWorld, hb.size);
    if (landed) playerOnStaticRail = true;
  }
  if (!playerOnStaticRail) {
    for (const auto& hb : m_staticWorldHitboxes) {
      ...
      const Math::Vec2 hbWorld = {MIN_X + hb.localCenter.x,
                                  MIN_Y + hb.localCenter.y};
      if (SnapToTopSupport(player, currentHbCenter, playerHalfSize, hbWorld,
                           hb.size, Train::TrainHitboxKind::Solid,
                           crouchHeld)) {
  ...
  // 발판에서 벗어났으면 지면 플래그 해제 — 단 정적 레일 위에서는 점프
  // 가능하도록 유지
  if (!playerOnTrainSurface && !playerOnStaticRail) player.SetOnGround(false);

  UpdateCarTransport(fdt, player, currentHbCenter, carTransportInjectHeld,
                     ignoreCarInjectPulseCost, carTransportInjectForcedSlot);

  // 점프 직후 공중이어도 캐리 밴드 안이면 열차 속도에 맞춰 같이 이동(지면처럼
  // 느껴지게 함)
  {
    ...
    const bool inBand =
        HitboxInTrainCarryBand(tlNow, m_totalTrainWidth, hcL, hhL);
    if (playerOnTrainSurface)
      m_airborneFromTrain = false;
    else if (!player.IsOnGround() && inBand &&
             (m_prevPlayerOnTrain || m_airborneFromTrain))
  ...
  if (m_entryTimer >= 0.0f && m_trainState == TrainState::Stationary) {
    ...
    if (m_entryTimer >= TRAIN_DEPART_DELAY) {
      ...
      Logger::Instance().Log(Logger::Severity::Info,
                             "Train: Train is now moving!");
    }
  }

  // Tick the "departed" flash message
  if (m_departedMsgTimer > 0.0f) m_departedMsgTimer -= fdt;

  if (m_car5ValveHintTimer > 0.0f) m_car5ValveHintTimer -= fdt;

  // --- Train movement ---
  if (m_trainState == TrainState::Moving) {
    m_trainCurrentSpeed =
        std::min(TRAIN_SPEED, m_trainCurrentSpeed + TRAIN_ACCEL * fdt);
    ...
    const float speedRatio =
        (TRAIN_SPEED > 0.0f) ? (m_trainCurrentSpeed / TRAIN_SPEED) : 1.0f;
    ...
    const bool inCarryBand =
        HitboxInTrainCarryBand(twLeft, m_totalTrainWidth, hc, hh);

    if ((m_playerOnTrain || m_airborneFromTrain) && inCarryBand)
      player.SetPosition(player.GetPosition() + Math::Vec2{move, 0.0f});
  }

  // --- Train combat progression: 클리어 전에는 다음 칸 경계에서 막음, 끝
  // 칸에서는 오른쪽 낙사 방지 ---
  currentHbCenter = player.GetHitboxCenter();
  {
  ...
  if (currentHbCenter.x - playerHalfSize.x < MIN_X)
    player.SetPosition({player.GetPosition().x +
                            (MIN_X - (currentHbCenter.x - playerHalfSize.x)),
...
void Train::StartEntryTimer() {
  if (m_entryTimer < 0.0f)  // only start once
  {
    ...
    if (!m_carTransportDroneWaterCd.empty())
      std::fill(m_carTransportDroneWaterCd.begin(),
                m_carTransportDroneWaterCd.end(), 0.f);
    m_car2HidePhase = Car2HidePhase::None;
    m_car2HideSeqTime = 0.f;
    m_car2InsideCharge = 0.f;
    m_car2EnterSavedValid = false;
    ...
    m_car3SirenSpawnTimer = 0.f;
    m_car3SirenPendingShutdown = false;
    if (m_sirenDroneManager) m_sirenDroneManager->ClearAllDrones();
    ResetCarTransportSlotsToInitialState();
    Logger::Instance().Log(
        Logger::Severity::Info,
        "Train: Entry timer started – train departs in %.1f s",
        TRAIN_DEPART_DELAY);
    m_trainCheatCarUnlock = false;
  }
}
...
std::string Train::GetDepartureAnnouncementText() const {
  if (m_trainState == TrainState::Moving) {
    if (m_departedMsgTimer > 0.0f) return "The train is now moving!";
    return "";
  }
  ...
  if (m_entryTimer >= 0.0f && m_entryTimer < TRAIN_DEPART_DELAY) {
    ...
    const int secsLeft = std::clamp(
        static_cast<int>(std::ceil(TRAIN_DEPART_DELAY - m_entryTimer)), 1, 3);
...
// DrawFilledQuad
// ---------------------------------------------------------------------------
void Train::DrawFilledQuad(Shader& colorShader, Math::Vec2 center,
                           Math::Vec2 size, float r, float g, float b,
                           float a) const {
  if (!m_skyVAO) return;

  Math::Matrix model =
      Math::Matrix::CreateTranslation(center) * Math::Matrix::CreateScale(size);
...
// ---------------------------------------------------------------------------
void Train::DrawBackground(Shader& colorShader, Math::Vec2 cameraPos,
                           float viewHalfW) const {
  ...
  // When the camera follows the player upward (car 4 stacks), shift the sky
  // vertically with the camera so the sunset bands still fill the frame instead
  // of leaving cleared black at the top.
  ...
  DrawFilledQuad(colorShader, {skyPx, relY(0.90f)}, {spanW, HEIGHT * 0.22f},
                 0.13f, 0.05f, 0.19f, 1.0f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.75f)}, {spanW, HEIGHT * 0.22f},
                 0.22f, 0.08f, 0.20f, 0.95f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.60f)}, {spanW, HEIGHT * 0.20f},
                 0.38f, 0.11f, 0.18f, 0.90f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.47f)}, {spanW, HEIGHT * 0.18f},
                 0.58f, 0.17f, 0.14f, 0.88f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.36f)}, {spanW, HEIGHT * 0.16f},
                 0.80f, 0.28f, 0.11f, 0.85f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.25f)}, {spanW, HEIGHT * 0.18f},
                 0.53f, 0.18f, 0.10f, 0.70f);
  DrawFilledQuad(colorShader, {skyPx, relY(0.11f)}, {spanW, HEIGHT * 0.22f},
                 0.10f, 0.07f, 0.08f, 1.0f);
  ...
  DrawFilledQuad(colorShader, {sunX, sunY}, {HEIGHT * 0.34f, HEIGHT * 0.34f},
                 1.00f, 0.48f, 0.18f, 0.28f);
  DrawFilledQuad(colorShader, {sunX, sunY}, {HEIGHT * 0.18f, HEIGHT * 0.18f},
                 1.00f, 0.62f, 0.24f, 0.58f);
  DrawFilledQuad(colorShader, {sunX, sunY}, {HEIGHT * 0.09f, HEIGHT * 0.09f},
                 1.00f, 0.79f, 0.35f, 0.95f);

  // Repeated cloud objects (dense, multi-row, infinite-style coverage)
  const float cloudBase = farPx;
  const float cloudStep = 620.0f;
  const int cloudMinI = static_cast<int>(
      std::floor((visibleLeft - cloudBase - 700.0f) / cloudStep));
  const int cloudMaxI = static_cast<int>(
      std::ceil((visibleRight - cloudBase + 700.0f) / cloudStep));
  for (int i = cloudMinI; i <= cloudMaxI; ++i) {
    ...
    DrawFilledQuad(colorShader, {x, y1}, {520.0f, 52.0f}, 0.40f, 0.17f, 0.27f,
                   0.26f);
    DrawFilledQuad(colorShader, {x + 120.0f, y1 - 24.0f}, {360.0f, 38.0f},
                   0.33f, 0.13f, 0.24f, 0.20f);

    DrawFilledQuad(colorShader, {x - 80.0f, y2}, {430.0f, 42.0f}, 0.52f, 0.21f,
                   0.20f, 0.18f);
    DrawFilledQuad(colorShader, {x + 50.0f, y2 - 20.0f}, {300.0f, 30.0f}, 0.45f,
                   0.17f, 0.18f, 0.14f);

    DrawFilledQuad(colorShader, {x + 30.0f, y3}, {340.0f, 28.0f}, 0.68f, 0.26f,
                   0.16f, 0.10f);
  }
  ...
  const int midMinI =
      static_cast<int>(std::floor((visibleLeft - midPx - 300.0f) / midStep));
  const int midMaxI =
      static_cast<int>(std::ceil((visibleRight - midPx + 300.0f) / midStep));
  for (int i = midMinI; i <= midMaxI; ++i) {
    ...
    DrawFilledQuad(colorShader, {x, relY(0.13f) + h * 0.5f}, {w, h}, 0.10f,
                   0.06f, 0.09f, 0.95f);
  }
  ...
  const int nearMinI =
      static_cast<int>(std::floor((visibleLeft - nearPx - 250.0f) / nearStep));
  const int nearMaxI =
      static_cast<int>(std::ceil((visibleRight - nearPx + 250.0f) / nearStep));
  for (int i = nearMinI; i <= nearMaxI; ++i) {
    const float x = nearPx + i * 210.0f;
    const float h = 86.0f + static_cast<float>((i + 100) % 5) * 20.0f;
    DrawFilledQuad(colorShader, {x, relY(0.07f) + h * 0.5f}, {150.0f, h}, 0.07f,
                   0.05f, 0.06f, 1.0f);
  }
  ...
  const int poleMinI =
      static_cast<int>(std::floor((visibleLeft - nearPx - 120.0f) / poleStep));
  const int poleMaxI =
      static_cast<int>(std::ceil((visibleRight - nearPx + 120.0f) / poleStep));
...
// ---------------------------------------------------------------------------
// DrawCarTransportOverlays — 시동 ON: PulseLine 스프라이트 / 시동 OFF:
// Start.png
// ---------------------------------------------------------------------------
void Train::DrawCarTransportOverlays(Shader& textureShader,
                                     Math::Vec2 cameraPos,
                                     float viewHalfW) const {
  ...
  for (int i = 0; i < kCarTransportCount; ++i) {
    ...
    if (slot.engineOn && !slot.skipPulseLineOverlay) {
      ...
      Background* tex = useLeft ? m_carTransportPulseLeftTex.get()
                                : m_carTransportPulseRightTex.get();
      if (!tex || tex->GetWidth() <= 0) continue;
      ...
      const float texAspect = static_cast<float>(tex->GetHeight()) /
                              static_cast<float>(tex->GetWidth());
      const Math::Vec2 scale{scaleW, scaleW * texAspect};

      const Math::Matrix model = Math::Matrix::CreateTranslation(pulseCenter) *
                                 Math::Matrix::CreateScale(scale);
      tex->Draw(textureShader, model);
    } else if (!slot.engineOn && m_carTransportStartTex &&
               m_carTransportStartTex->GetWidth() > 0) {
      constexpr float iconW = 110.f;
      const float aspect =
          static_cast<float>(m_carTransportStartTex->GetHeight()) /
          static_cast<float>(m_carTransportStartTex->GetWidth());
      const Math::Matrix model =
          Math::Matrix::CreateTranslation(wc) *
          Math::Matrix::CreateScale({iconW, iconW * aspect});
...
// ---------------------------------------------------------------------------
void Train::DrawCarTransportVFX(Shader& colorShader, Math::Vec2 cameraPos,
                                float viewHalfW) const {
  if (!m_skyVAO) return;
  ...
  for (int i = 0; i < kCarTransportCount; ++i) {
    ...
    if (!s.engineOn && s.engineGlowTimer <= 0.f) continue;

    const Math::Vec2 wc = {trainLeft + s.localCenter.x,
                           MIN_Y + s.localCenter.y};
    if (wc.x + s.halfSize.x < visLeft || wc.x - s.halfSize.x > visRight)
      continue;

    const float flash = (s.engineGlowTimer > 0.f)
                            ? std::min(1.f, s.engineGlowTimer * 2.f)
                            : 0.f;
    const float baseA = s.engineOn ? 0.20f : 0.f;
    const float a = std::min(1.f, baseA + flash * 0.55f);

    DrawFilledQuad(colorShader, wc,
                   {s.halfSize.x * 1.65f, s.halfSize.y * 0.52f}, 0.22f, 0.82f,
                   1.0f, a);
    if (flash > 0.02f)
      DrawFilledQuad(colorShader, wc,
                     {s.halfSize.x * 2.15f, s.halfSize.y * 0.82f}, 0.35f, 0.92f,
                     1.0f, flash * 0.38f);
  }
}

void Train::DrawValveWaterVFX(Shader& colorShader,
                              const Math::Matrix& worldProjection,
                              Math::Vec2 cameraPos, float viewHalfW) const {
  (void)colorShader;

  if (m_valveWaterParticles.empty() && m_valvePressureT <= 0.01f) return;
...
// ---------------------------------------------------------------------------
void Train::DrawRailTrack(Shader& shader, Math::Vec2 cameraPos,
                          float viewHalfW) const {
  if (!m_railTile || m_railTileW <= 0.0f) return;
  ...
  const int minTile =
      static_cast<int>(std::floor((leftX - tileLeft0) / m_railTileW)) - 1;
  const int maxTile =
      static_cast<int>(std::ceil((rightX - tileLeft0) / m_railTileW)) + 1;
...
void Train::Draw(Shader& shader, Math::Vec2 cameraPos, float viewHalfW) const {
  ...
  if (m_thirdThirdTrain) {
    float cx = trainLeft + m_car1Width + m_car2Width + m_car3Width +
               m_car4Width * 0.5f;
    float cy = MIN_Y + HEIGHT * 0.5f;
    Math::Matrix model = Math::Matrix::CreateTranslation({cx, cy}) *
  ...
  if (m_valveSprite && m_valveSprite->GetWidth() > 0) {
    const Math::Vec2 valveWorld = {trainLeft + m_valveLocalCenter.x,
                                   MIN_Y + m_valveLocalCenter.y};
    const float cwDeg = -m_valveOpenT * 115.0f;
    Math::Matrix model = Math::Matrix::CreateTranslation(valveWorld) *
  ...
  for (const auto& robot : m_robots) {
    if (!robot.IsDead()) robot.Draw(shader);
  }
}
...
void Train::DrawDrones(Shader& shader) const {
  if (m_droneManager) m_droneManager->Draw(shader);
  if (m_carTransportDroneManager) m_carTransportDroneManager->Draw(shader);
  if (m_sirenDroneManager) m_sirenDroneManager->Draw(shader);
}

void Train::DrawRadars(const Shader& colorShader,
                       DebugRenderer& debugRenderer) const {
  if (m_droneManager) m_droneManager->DrawRadars(colorShader, debugRenderer);
  if (m_carTransportDroneManager)
...
void Train::DrawGauges(Shader& colorShader,
                       DebugRenderer& debugRenderer) const {
  if (m_droneManager) m_droneManager->DrawGauges(colorShader, debugRenderer);
  if (m_carTransportDroneManager)
    m_carTransportDroneManager->DrawGauges(colorShader, debugRenderer);
  if (m_sirenDroneManager)
    m_sirenDroneManager->DrawGauges(colorShader, debugRenderer);
  for (const auto& robot : m_robots) {
    if (!robot.IsDead()) robot.DrawGauge(colorShader, debugRenderer);
  }
}
...
void Train::DrawDebug(Shader& colorShader, DebugRenderer& debugRenderer) const {
  ...
  for (const auto& source : m_pulseSources)
    debugRenderer.DrawBox(colorShader, source.GetPosition(), source.GetSize(),
                          {0.0f, 1.0f});
  ...
  // 월드 고정 레일 발판 (orange)
  for (const auto& hb : m_staticWorldHitboxes) {
    const Math::Vec2 worldPos = {MIN_X + hb.localCenter.x,
                                 MIN_Y + hb.localCenter.y};
  ...
  // Hiding spots (green) — move with train, same as Hallway display convention
  {
    const float trainLeft = MIN_X + m_trainOffset;
    for (const auto& spot : m_hidingSpots) {
      Math::Vec2 worldPos = {trainLeft + spot.localCenter.x,
                             MIN_Y + spot.localCenter.y};
      debugRenderer.DrawBox(colorShader, worldPos, spot.size, {0.3f, 1.0f});
    }
  }

  // Train hitboxes (cyan) — shrink display by 2px so stacked box edges don't
  // merge
  const float trainLeft = MIN_X + m_trainOffset;
  constexpr float kDebugShrink = 2.0f;
  for (const auto& hb : m_trainHitboxes) {
    Math::Vec2 worldPos = {trainLeft + hb.localCenter.x,
                           MIN_Y + hb.localCenter.y};
    Math::Vec2 displaySize = {hb.size.x - kDebugShrink,
                              hb.size.y - kDebugShrink};
    debugRenderer.DrawBox(colorShader, worldPos, displaySize, 0.0f, 1.0f, 1.0f);
  }

  // Map boundary markers (white)
  const float bndH = HEIGHT;
  debugRenderer.DrawBox(colorShader, {MIN_X, MIN_Y + bndH * 0.5f},
                        {10.0f, bndH}, {1.0f, 1.0f});
  debugRenderer.DrawBox(colorShader,
                        {MIN_X + m_totalTrainWidth, MIN_Y + bndH * 0.5f},
                        {10.0f, bndH}, {1.0f, 1.0f});
}
...
void Train::Shutdown() {
  ...
  if (m_carTransportDroneManager) m_carTransportDroneManager->Shutdown();
  if (m_sirenDroneManager) m_sirenDroneManager->Shutdown();

  if (m_car2EnterPromptTex) m_car2EnterPromptTex->Shutdown();
  if (m_car2LeavePromptTex) m_car2LeavePromptTex->Shutdown();
  if (m_carTransportPulseLeftTex) m_carTransportPulseLeftTex->Shutdown();
  if (m_carTransportPulseRightTex) m_carTransportPulseRightTex->Shutdown();
  if (m_carTransportStartTex) m_carTransportStartTex->Shutdown();
...
// Drone accessors
// ---------------------------------------------------------------------------
const std::vector<Drone>& Train::GetDrones() const {
  return m_droneManager->GetDrones();
}
std::vector<Drone>& Train::GetDrones() { return m_droneManager->GetDrones(); }

const std::vector<Drone>& Train::GetSirenDrones() const {
  return m_sirenDroneManager->GetDrones();
}

void Train::ClearAllDrones() {
  if (m_droneManager) m_droneManager->ClearAllDrones();
  if (m_carTransportDroneManager) m_carTransportDroneManager->ClearAllDrones();
  if (m_sirenDroneManager) m_sirenDroneManager->ClearAllDrones();
}
